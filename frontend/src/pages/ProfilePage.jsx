...
export default function ProfilePage() {
    ...
    const rightPanel = (
        <div className="p-[18px]">
            <h3 className="text-text-secondary text-[13px] font-body mb-4 tracking-[0.3px]">Gợi ý theo dõi</h3>
        ...
        <PageLayout rightPanel={rightPanel}>
            <div className="flex-1 overflow-auto">
                ...
                <div className="flex items-center gap-3 px-5 py-3.5 border-b border-border sticky top-0 bg-base z-10">
                    ...
                    <button className="bg-transparent border-none text-text-secondary cursor-pointer text-xl">···</button>
                </div>
                ...
                <div className="relative">
                    <div className="h-[180px] overflow-hidden relative"
                        ...
                        <div className="absolute -top-10 -right-10 w-[200px] h-[200px] rounded-full" style={{ background: 'radial-gradient(circle,rgba(255,45,120,.15),transparent 70%)' }} />
                    </div>
                ...
                <div className="px-8 pt-16 pb-6">
                    <div className="flex items-baseline gap-2 mb-1">
                        <h1 className="font-display font-bold text-[22px] text-white m-0">{profile.fullName}</h1>
                        {profile.isCreator && (
                            <span className="text-transparent bg-clip-text bg-brand-gradient text-[13px] font-semibold font-body">✦ Creator</span>
                        )}
                    </div>
                    ...
                    {profile.bio && (
                        <p className="text-text-secondary text-[13px] font-body leading-relaxed max-w-[480px] mb-4">{profile.bio}</p>
                    )}
...
function VideoThumb({ video }) {
    ...
    return (
        <div className="relative rounded cursor-pointer overflow-hidden transition-transform duration-200"
            style={{ aspectRatio: '9/16', background: `linear-gradient(135deg,hsl(${hue},25%,10%),hsl(${(hue + 60) % 360},15%,6%))`, transform: hovered ? 'scale(1.03)' : 'scale(1)' }}
...
function BackIcon() {
    return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="#888" strokeWidth="1.5" strokeLinecap="round">
        <path d="M10 13L5 8L10 3" />
    </svg>;
}
function ShareSmIcon() {
    return <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#888" strokeWidth="1.2">
