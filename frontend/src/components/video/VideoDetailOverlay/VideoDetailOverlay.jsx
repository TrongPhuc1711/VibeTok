 ...
 * Props:
 *  videoId          – string
 *  highlightComment – boolean (tự cuộn tới phần comment, e.g. từ notif comment)
 *  onClose          – () => void
 */
export default function VideoDetailOverlay({ videoId, highlightComment = false, onClose }) {
  ...
  useEffect(() => {
    ...
    getVideoById(videoId)
      .then(r => {
        ...
        setLiked(getLikedSet().has(String(v.id)));
        setFollowing(v.user?.isFollowing ?? false);
      })
      .catch(() => setVideo(null))
  ...
  /* auto-scroll to comments if highlightComment */
  useEffect(() => {
...
function CommentRow({ comment, highlight = false }) {
  ...
  return (
    <div
      className={`flex gap-3 py-3 rounded-xl transition-colors ${
        highlight ? 'bg-[#ff2d78]/8 px-2' : ''
      }`}
