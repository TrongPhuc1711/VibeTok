...
// Bookmarks in localStorage (no API yet)
...
export default function VideoCardActions({ video, onComment, onShare, onBookmark, inline = false }) {
  ...
  // ✅ All state sourced from DB (video.isLiked, video.user.isFollowing) - NO localStorage for likes
  ...
  // Sync state when video changes (e.g., navigating between videos)
  useEffect(() => {
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      showError('Thao tác thất bại', 'Thử lại sau');
    } finally {
  ...
  const handleBookmark = () => {
    ...
    if (now) showSuccess('Đã lưu video', 'Thêm vào danh sách lưu');
  ...
  const handleShare = () => {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(url).then(() => {
        showSuccess('Đã sao chép link!', 'Chia sẻ với bạn bè');
      }).catch(() => {
        showInfo('Link video', url);
      });
    }
    onShare?.(videoId);
  };
    ...
    <>
      <div className={wrapperCls}>
        ...
        <ActionBtn
          ...
          inline={inline}
          ariaLabel={bookmarked ? 'Bỏ lưu' : 'Lưu video'}
        />
