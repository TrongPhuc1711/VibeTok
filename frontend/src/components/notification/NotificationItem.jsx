...
import { HeartIcon, CommentIcon,MusicIcon,UserIcon } from '../../icons/ActionIcons';
const TYPE_CONFIG = {
  like:    { icon:HeartIcon , text: (a) => `${a} đã thích video của bạn` },
  follow:  { icon:UserIcon , text: (a) => `${a} đã bắt đầu theo dõi bạn` },
  comment: { icon: CommentIcon, text: (a) => `${a} đã bình luận về video của bạn` },
  mention: { icon: '@',  text: (a) => `${a} đã nhắc đến bạn trong bình luận` },
  duet:    { icon: MusicIcon, text: (a) => `${a} đã tạo duet với video của bạn` },
};
  ...
  return (
    <div
      ...
      className={`
        ...
        ${notif.read ? 'hover:bg-white/[0.02]' : 'bg-[#ff2d78]/[0.04] hover:bg-[#ff2d78]/[0.07]'}
      `}
    >
      {/* Avatar actor */}
      <div className="relative shrink-0">
        ...
        {/* Type icon badge */}
        <div className="absolute -bottom-1 -right-1 w-[18px] h-[18px] rounded-full bg-[#111118] border border-[#1e1e2e] flex items-center justify-center text-[9px]">
          {cfg.icon}
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-body leading-relaxed m-0">
          ...
          <span className="text-[#aaa]"> {cfg.text('').replace(name, '').trim()}</span>
        </p>
        {notif.meta?.comment && (
