...
export const repostVideo = async (req, res) => {
    try {
        ...
        // Toggle: nếu đã repost thì unrepost, ngược lại
        const alreadyReposted = await VideoModel.isReposted(userId, videoId);

        if (alreadyReposted) {
            await VideoModel.unrepost(userId, videoId);
            res.json({ reposted: false, message: 'Đã bỏ đăng lại' });
        } else {
            await VideoModel.repost(userId, videoId);

            // Gửi notification cho chủ video (fire-and-forget)
            if (String(video.userId) !== String(userId)) {
                UserModel.findById(userId).then(async (senderDb) => {
                    const sender = senderDb ? normalizeUser(senderDb) : req.user;
                    await triggerNotification(video.userId, sender, 'repost', video.id);
                }).catch(() => { });
            }

            res.json({ reposted: true, message: 'Đã đăng lại!' });
        }
    } catch (e) {
        console.error('repostVideo error:', e);
        res.status(500).json({ message: 'Lỗi đăng lại video', error: e.message });
    }
};

// GET /api/videos/user/:userId/reposts
export const getRepostedVideos = async (req, res) => {
    try {
        const page = Math.max(1, parseInt(req.query.page) || 1);
        const limit = Math.min(50, Math.max(1, parseInt(req.query.limit) || 12));
        const currentUserId = req.user?.id ?? null;
        const videos = await VideoModel.getRepostsByUserId(req.params.userId, { page, limit, currentUserId });
        res.json({ videos });
    } catch (e) {
